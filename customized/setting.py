...
class testing:
    def __init__( self, mode, level):
        self.mode = mode   # 0: no testing , 1: control via browser (case has stage in database), 2: with jenkins 
...
class Setting:
    ...
    def __init__( self, typeList, caseList, configurationList, operationType, preselectedOperation, testing, mySQL_struct ):
    ...
    def getColumnForCase( self, caseName, columnName ):
        return self.__gateToMySQL.getColumnEntry( 'cases', self.__gateToMySQL.getIdFromName( 'cases', caseName ), columnName ) 
    ...
    def selectId( self, table, lists_inst ):
        ...
        if not(self.__testing.mode == '0'):
            # TESTLEVEL: preselect anything but examples (types, cases, configurations) and specify state (for jsp) or level (for jenkins) in database
    ...
    def setNumData( self, simData, type, case, configuration ):
        ...
        columnEntry = self.__gateToMySQL.getColumnEntry( 'cases', self.__gateToMySQL.getIdFromName( 'cases', case ), 'solver_flow_' + configuration ) 
        if( columnEntry != 'None' ): 
            solver.append( self.convertNumericsDbEntry2specifcation( configuration, columnEntry, 'solver' ) ) 
         
        columnEntry = self.__gateToMySQL.getColumnEntry( 'cases', self.__gateToMySQL.getIdFromName( 'cases', case ), 'solver_mass_' + configuration )
        if( columnEntry != 'None'  ): 
            solver.append( self.convertNumericsDbEntry2specifcation( configuration, columnEntry, 'solver' ) )                            

        columnEntry =  self.__gateToMySQL.getColumnEntry( 'cases', self.__gateToMySQL.getIdFromName( 'cases', case ), 'solver_heat_' + configuration )
        if( columnEntry != 'None'  ): 
            solver.append( self.convertNumericsDbEntry2specifcation( configuration, columnEntry, 'solver' ) ) 
        
        columnEntry = self.__gateToMySQL.getColumnEntry( 'cases', self.__gateToMySQL.getIdFromName( 'cases', case ), 'preconditioner_flow_' + configuration )
        if( columnEntry != 'None'  ): 
            preconditioner.append( self.convertNumericsDbEntry2specifcation( configuration, columnEntry, 'preconditioner' ) ) 
        
        columnEntry = self.__gateToMySQL.getColumnEntry( 'cases', self.__gateToMySQL.getIdFromName( 'cases', case ), 'preconditioner_mass_' + configuration ) 
        if( columnEntry != 'None'  ): 
            preconditioner.append( self.convertNumericsDbEntry2specifcation( configuration, columnEntry, 'preconditioner' ) )                            

        columnEntry = self.__gateToMySQL.getColumnEntry( 'cases', self.__gateToMySQL.getIdFromName( 'cases', case ), 'preconditioner_heat_' + configuration )
        if( columnEntry != 'None'  ): 
            preconditioner.append( self.convertNumericsDbEntry2specifcation( configuration, columnEntry, 'preconditioner' ) ) 

        columnEntry = self.__gateToMySQL.getColumnEntry( 'cases', self.__gateToMySQL.getIdFromName( 'cases', case ), 'theta_flow' )
        if( columnEntry != 'None'  ): 
            theta.append ( columnEntry ) 

        columnEntry = self.__gateToMySQL.getColumnEntry( 'cases', self.__gateToMySQL.getIdFromName( 'cases', case ), 'theta_mass' )
        if( columnEntry != 'None'  ): 
            theta.append ( columnEntry ) 

        columnEntry = self.__gateToMySQL.getColumnEntry( 'cases', self.__gateToMySQL.getIdFromName( 'cases', case ), 'theta_heat' ) 
        if( columnEntry != 'None'  ): 
            theta.append ( columnEntry ) 
    ...
    #################################################################
    #  Setting: SetItemsList  
    #  Task:
    #     generate examples Lists to loop over in Environment run
    #
        
    def SetItemsLists( self, operationType, operation, computer ):  
        
        if operationType == 'b': # building (only configuration)
            self.setTypeList( [None] ) 
            self.setCaseList( [[None]] )                          
            self.setConfigurationList( self.setItemConstituents( groupType = 'configurations',
                                                                         computerOfSubject = computer ) )                        
        elif operationType == 's':  # simulating (all item constituents (type, case, configuration))
            self.setTypeList( self.setItemConstituents( groupType = 'types' ) ) 
            self.setCaseList( self.setItemConstituents( groupType = 'cases' ) )                                      
            self.setConfigurationList( self.setItemConstituents( groupType = 'configurations',
                                                                      computerOfSubject = computer ) ) 
            ...
            if operation == 'j' or operation == 'w': # generate or wait for JPG (only type)
                self.setCaseList( [[None]] ) 
                self.setConfigurationList( [None] ) 
